...
let currentPos = [centralport[0], centralport[1]];
const touchedCoords = [];
// calculate manhatten distances
const distance = (coords) => Math.abs(centralport[0] - coords.x) + Math.abs(centralport[1] - coords.y);

function draw(coords, id) {
    let length = 0;
    function move(up, left, updateCurrentPos) {
        const coord = { x: currentPos[0] + left, y: currentPos[1] + up, n: 0, length: ++length, id }
        const existingCoord = touchedCoords.findIndex(c => c.x == coord.x && c.y == coord.y);
        if (existingCoord !== -1) {
            touchedCoords[existingCoord].n++;
            touchedCoords[existingCoord].length += coord.length;
            touchedCoords[existingCoord].id = touchedCoords[existingCoord].id.concat(id);
        } else {
            touchedCoords.push(coord);
        }
        if (updateCurrentPos)
            currentPos = [coord.x, coord.y]
    }
    coords.split(',').forEach(c => {
        const stepsize = +c.slice(1);
        switch (c[0]) {
            case 'R':
                for(let i = 1; i <= stepsize; i++) move(0, i, i == stepsize);
                break;
            case 'L':
                for(let i = 1; i <= stepsize; i++) move(0, -i, i == stepsize);
                break;
            case 'U':
                for(let i = 1; i <= stepsize; i++) move(-i, 0, i == stepsize);
                break;
            case 'D':
                for(let i = 1; i <= stepsize; i++) move(i, 0, i == stepsize);
                break;
